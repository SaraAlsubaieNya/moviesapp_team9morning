...
final class UserService {
    ...
    private let baseURL = URL(string: "https://api.airtable.com/v0/appsfcB6YESLj4NCN/users")!
    ...
    func fetchUsers() async throws -> [AirtableUsersResponse.Record] {
        ...
        if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode != 200 {
            ...
            throw NSError(domain: "UserService", code: httpResponse.statusCode, userInfo: [
                NSLocalizedDescriptionKey: "Server returned status \(httpResponse.statusCode): \(responseString)"
            ])
        }

        let decoded = try JSONDecoder().decode(AirtableUsersResponse.self, from: data)
